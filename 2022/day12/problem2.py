...
def main():
    ...
    steps_by_position = solve_maze(maze, end)
    steps = find_lowest_start(maze, steps_by_position)
    print(steps)
...
def find_lowest_start(maze, steps_by_position):
    starts = [k for k, v in maze.items() if v == "a" or v == "S"]
    return min(steps_by_position[x] for x in starts if x in steps_by_position)
